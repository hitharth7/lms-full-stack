...
const MyCourses = () => {
  ...
  const { myCourses } = getEducatorMetrics();
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-sm text-gray-600">
                {myCourses.map((course) => {
                  ...
                  const courseEarnings = (finalPrice * enrolledCount).toFixed(2);

                  return (
                    <tr key={course._id} className="hover:bg-gray-50/30 transition-colors">
                      <td className="px-6 py-4 flex items-center gap-3">
                        {/* Mini Visual Gradient */}
                        <div 
                          style={{ background: course.courseThumbnail }}
                          className="w-12 h-10 rounded-lg flex-shrink-0 flex items-center justify-center text-white text-[8px] font-black p-1 text-center relative overflow-hidden"
                        >
                          <div className="absolute inset-0 bg-black/5 opacity-20"></div>
                          <span className="relative z-10 truncate max-w-full">{course.category}</span>
                        </div>
                        <div>
                      ...
                      <td className="px-6 py-4 font-semibold text-gray-700">{enrolledCount}</td>
                      <td className="px-6 py-4 font-extrabold text-gray-900">${courseEarnings}</td>
                      <td className="px-6 py-4 font-medium text-gray-500">
                        {course.chapters.length} sections &bull; {course.lecturesCount} lectures
                      </td>
