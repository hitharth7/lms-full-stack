...
const DEFAULT_AUTH_USERS = [DEFAULT_AUTH_USER];
...
const getCourseStatsFromChapters = (chapters = []) => {
  return chapters.reduce(
    (totals, chapter) => {
      const lectures = chapter.chapterContent || [];
      totals.lecturesCount += lectures.length;
      totals.duration += lectures.reduce((sum, lecture) => sum + (Number(lecture.lectureDuration) || 0), 0);
      return totals;
    },
    { duration: 0, lecturesCount: 0 }
  );
};

const normalizeCourseStats = (course) => {
  const stats = getCourseStatsFromChapters(course.chapters);
...
const normalizeCourses = (courses = []) => courses.map(normalizeCourseStats);

export const AppContextProvider = ({ children }) => {
  // --- STATE DECLARATIONS ---
  // Load courses
  const [courses, setCourses] = useState(() => {
    return normalizeCourses(loadJSON("lms_courses", INITIAL_COURSES));
  });

  // Load auth session and registered users
  const [authUser, setAuthUser] = useState(() => {
    return loadJSON("lms_auth_user", null);
  });

  const [registeredUsers, setRegisteredUsers] = useState(() => {
    return loadJSON("lms_users", DEFAULT_AUTH_USERS);
  });

  // Load user role ('student' or 'educator')
  const [userRole, setUserRole] = useState(() => {
    return authUser?.role || localStorage.getItem("lms_user_role") || "student";
  });
  ...
  useEffect(() => {
    if (authUser) {
      localStorage.setItem("lms_auth_user", JSON.stringify(authUser));
    } else {
  ...
  useEffect(() => {
    ...
    const registeredUser = registeredUsers.find((user) => user.email.toLowerCase() === authUser.email.toLowerCase());
    if (!registeredUser) return;

    if (registeredUser.role !== authUser.role || userRole !== registeredUser.role) {
      setAuthUser(registeredUser);
      setUserRole(registeredUser.role || "student");
    }
  }, [authUser, registeredUsers, userRole]);

  useEffect(() => {
    localStorage.setItem("lms_users", JSON.stringify(registeredUsers));
  }, [registeredUsers]);
  ...
  const login = ({ email, password }) => {
    const matchedUser = registeredUsers.find((user) => user.email.toLowerCase() === email.toLowerCase());
    ...
    setUserRole(matchedUser.role || "student");
    return { success: true };
  };
  ...
  const resetDatabase = () => {
    setCourses(normalizeCourses(INITIAL_COURSES));
    setEnrolledCourseIds([]);
    setCompletedLectures({});
    setUserRole("student");
    setAuthUser(null);
    setRegisteredUsers(DEFAULT_AUTH_USERS);
  };
